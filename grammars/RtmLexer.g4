...
SOURCE_DESC_FULL_LINE_COMMENT: FULL_LINE_COMMENT -> type(FULL_LINE_COMMENT), pushMode(SOURCE);
...
ERROR_CHAR: .; //If no other tokens match. Ensures lexer does not fail and error can be handled in parser.
...
DATA_IDENTIFIER_OR_MASK: '@'? [a-zA-Z] [\-a-zA-Z0-9.]* -> type(IDENTIFIER);
...
CODE_STRING_INLINE_COMMENT:
	WHITESPACE? '<<' INPUTCHARACTER* NEWLINE -> channel(COMMENTS_CHANNEL), type(INLINE_COMMENT), pushMode(CODE_STRING_MODE2);
...
CODE_STRING_INLINE_COMMENT_2:
	WHITESPACE INLINE_COMMENT -> channel(COMMENTS_CHANNEL), type(INLINE_COMMENT), pushMode(COMMENT_MODE_SKIP_NEWLINE);
...
FULL_LINE_COMMENT_AFTER_SKIP:
	FULL_LINE_COMMENT -> type(FULL_LINE_COMMENT), channel(COMMENTS_CHANNEL), popMode;
COMMENT_MODE_SKIP_NEWLINE_ERROR_CHAR: ERROR_CHAR -> type(ERROR_CHAR);
